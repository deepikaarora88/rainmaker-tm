...
public class IdGenerationService {
    ...
    public boolean autoCreateNewSeq=true;
    ...
    public IdGenerationResponse generateIdResponse(IdGenerationRequest idGenerationRequest) throws Exception {
        ...
        for (IdRequest idRequest : idRequests) {
            List<String> generatedId = generateIdFromIdRequest(idRequest);
        ...
        idGenerationResponse.setResponseInfo(responseInfoFactory.createResponseInfoFromRequestInfo(null,true));
     ...
     * @throws Exception
     */
    private List generateIdFromIdRequest(IdRequest idRequest) throws Exception {
        ...
        return getFormattedId(idRequest,autoCreateNewSeqFlag);
    }
    ...
    private List getFormattedId(IdRequest idRequest, boolean autoCreateNewSeqFlag) throws Exception {
            ...
            for (String attributeName : matchList) {
                ...
                if (attributeName.substring(0, 3).equalsIgnoreCase("seq")) {
                    if (!sequences.containsKey(attributeName)) {
                        sequences.put(attributeName, generateSequenceNumber(attributeName, idRequest,autoCreateNewSeqFlag));
                    }
                    ...
                    idFormat = idFormat.replace("[" + attributeName + "]",
                            generateFinancialYearDateFormat(attributeName));
                } else if (attributeName.substring(0, 2).equalsIgnoreCase("cy")) {
                    idFormat = idFormat.replace("[" + attributeName + "]",
                            generateCurrentYearDateFormat(attributeName));
                }  else {
                    idFormat = idFormat.replace("[" + attributeName + "]", generateRandomText(attributeName));
                }
            }
     ...
     */
    private String generateFinancialYearDateFormat(String financialYearFormat) {
        try {
     ...
     * @return formattedDate
     */
    private String generateCurrentYearDateFormat(String dateFormat) {
        try {
     ...
     * @return randomTxt
     */
    private String generateRandomText(String regex) {
     ...
     * @return seqNumber
     */
    private List<String> generateSequenceNumber(String sequenceName, IdRequest idRequest,boolean autoCreateNewSeqFlag) throws Exception {
        Integer count = getCount(idRequest);
